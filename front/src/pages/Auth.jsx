...
function KakaoButton({ onClick }) {
  return (
    <button onClick={onClick} style={{ width: '100%', height: 52, borderRadius: 14, background: '#FEE500', color: '#191600', fontSize: 15, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 9 }}>
  ...
  return (
    <div data-screen-label="로그인" className="nd-no-scrollbar" style={{ flex: 1, overflow: 'auto', background: T.bg }}>
      <div style={{ padding: '60px 26px 0', textAlign: 'center' }}>
        ...
        <h1 style={{ margin: '16px 0 0', fontSize: 23, fontWeight: 800, letterSpacing: '-0.02em', color: T.ink }}>다시 만나 반가워요</h1>
        <p style={{ margin: '8px 0 0', fontSize: 13.5, color: T.inkSoft }}>검진AI에 로그인하고 내 건강을 확인하세요</p>
      </div>
      ...
      <div style={{ padding: '28px 26px 0', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ textAlign: 'right', marginTop: -2 }}>
          <button onClick={() => onNav('reset')} style={{ fontSize: 12.5, fontWeight: 600, color: T.inkMid, padding: 2 }}>비밀번호 찾기</button>
        </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '22px 26px 18px' }}>
        ...
        <span style={{ fontSize: 12, color: T.inkSoft, fontWeight: 600 }}>또는</span>
        <div style={{ flex: 1, height: 1, background: T.line }} />
      </div>
      <div style={{ padding: '0 26px' }}><KakaoButton onClick={kakaoLogin} /></div>

      <div style={{ textAlign: 'center', padding: '26px 26px 30px', fontSize: 13.5, color: T.inkMid }}>
        아직 회원이 아니신가요?{' '}
        <button onClick={() => onNav('signup')} style={{ fontSize: 13.5, fontWeight: 800, color: T.blue }}>회원가입</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div data-screen-label="회원가입" className="nd-no-scrollbar" style={{ flex: 1, overflow: 'auto', background: T.bg }}>
      ...
      <div style={{ padding: '4px 26px 0' }}>
        <p style={{ margin: 0, fontSize: 13.5, color: T.inkSoft }}>몇 가지 정보만 입력하면 바로 시작해요</p>
      </div>
      ...
      <div style={{ padding: '20px 26px 0', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div>
          ...
          {err.gender && <div style={{ fontSize: 11.5, color: T.danger, fontWeight: 600, margin: '6px 0 0 4px' }}>{err.gender}</div>}
        </div>
      </div>
      ...
      <div style={{ margin: '20px 26px 0', padding: 16, borderRadius: 16, background: '#fff', border: '1px solid ' + (err.agree ? T.danger : T.line) }}>
        <button onClick={toggleAll} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 11, paddingBottom: 12, borderBottom: '1px solid ' + T.line, textAlign: 'left' }}>
          <Check on={agreeAll} big /><span style={{ fontSize: 14.5, fontWeight: 800, color: T.ink }}>약관 전체 동의</span>
        </button>
        {[
          ...
          <button key={it.k} onClick={() => toggle(it.k)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 11, padding: '9px 0', textAlign: 'left' }}>
            <Check on={agree[it.k]} />
            <span style={{ flex: 1, fontSize: 13.5, fontWeight: 500, color: T.inkMid }}><span style={{ color: T.blue, fontWeight: 700 }}>[필수] </span>{it.label}</span>
            <Icon name="chevR" size={16} color="#C6D3E6" />
          </button>
        ))}
      </div>
      {err.agree && <div style={{ fontSize: 11.5, color: T.danger, fontWeight: 600, margin: '8px 0 0 30px' }}>{err.agree}</div>}
  ...
  return (
    <div data-screen-label="비밀번호 찾기" className="nd-no-scrollbar" style={{ flex: 1, overflow: 'auto', background: T.bg }}>
      ...
      <div style={{ padding: '12px 26px 0' }}>
        ...
        <h2 style={{ margin: 0, fontSize: 20, fontWeight: 800, color: T.ink, letterSpacing: '-0.02em' }}>비밀번호를 잊으셨나요?</h2>
        <p style={{ margin: '8px 0 0', fontSize: 13.5, color: T.inkMid, lineHeight: 1.6 }}>가입하신 이메일로 비밀번호 재설정 링크를 보내드릴게요.</p>
      </div>
      <div style={{ padding: '24px 26px 0', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        {sent && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '12px 14px', borderRadius: 13, background: T.greenSoft }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 600, color: T.ok }}>이메일을 발송했습니다. 메일함을 확인해주세요.</span>
          </div>
        )}
        <Button variant="primary" onClick={submit}>인증메일 발송</Button>
        <button onClick={() => onNav('login')} style={{ height: 50, fontSize: 14, fontWeight: 700, color: T.inkMid }}>로그인으로 돌아가기</button>
      </div>
    </div>
  );
}
