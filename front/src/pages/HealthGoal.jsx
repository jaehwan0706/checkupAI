...
function AddGoalModal({ open, onAdd, onClose }) {
  ...
  return (
    <Modal open={open} onClose={onClose}>
      <h3 style={{ margin: '0 0 14px', fontSize: 17, fontWeight: 800, color: T.ink }}>직접 목표 추가</h3>
...
function EditGoalModal({ goal, onSave, onClose }) {
  ...
  return (
    <Modal open={true} onClose={onClose}>
      <h3 style={{ margin: '0 0 18px', fontSize: 17, fontWeight: 800, color: T.ink }}>목표 수정</h3>

      <div style={{ marginBottom: 12 }}>
        <div style={{ fontSize: 12, fontWeight: 700, color: T.inkSoft, marginBottom: 6 }}>목표 이름</div>
      ...
      <div style={{ marginBottom: 20 }}>
        <div style={{ fontSize: 12, fontWeight: 700, color: T.inkSoft, marginBottom: 6 }}>세부 내용</div>
        <div style={{ display: 'flex', alignItems: 'center', height: 48, padding: '0 14px', borderRadius: 13, background: T.bg, border: '1.5px solid ' + T.line }}>
    ...
    <div data-screen-label="건강 목표" className="nd-no-scrollbar" style={{ flex: 1, overflow: 'auto', background: T.bg }}>
      ...
      <div style={{ padding: '4px 20px 0' }}>
        <div style={{ display: 'flex', gap: 10, padding: '13px 15px', borderRadius: 14, background: 'linear-gradient(135deg,#00B894,#00A382)' }}>
          ...
          <div>
            <div style={{ fontSize: 12.5, fontWeight: 800, color: 'rgba(255,255,255,0.9)', marginBottom: 2 }}>AI 추천 목표</div>
            <p style={{ margin: 0, fontSize: 12.5, lineHeight: 1.5, color: 'rgba(255,255,255,0.9)' }}>검진 수치를 분석해 맞춤 목표를 추천했어요.</p>
          </div>
        </div>
      </div>

      {loading ? (
        <div style={{ padding: '24px 20px', textAlign: 'center', color: T.inkSoft, fontSize: 14 }}>불러오는 중...</div>
      ) : (
        <div style={{ padding: '16px 20px 0', display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <span style={{ fontSize: 14.5, fontWeight: 800, color: T.ink }}>{g.title}</span>
                      {g.ai && <span style={{ fontSize: 10, fontWeight: 800, color: T.blue, background: T.blueSoft, padding: '2px 6px', borderRadius: 999 }}>AI</span>}
                    ...
                    <div style={{ fontSize: 12.5, color: T.inkMid, marginTop: 3, lineHeight: 1.5 }}>{g.detail}</div>
                  </div>
                </div>

                <GoalBar pct={g.pct} color={color} />
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 7 }}>
                  <span style={{ fontSize: 11.5, fontWeight: 700, color: done ? T.green : T.inkSoft }}>{done ? '목표 달성!' : '진행 중'}</span>
                  <span style={{ fontSize: 11.5, fontWeight: 800, color }}>{g.pct}%</span>
                </div>
                ...
                {menuOpen && !deleteOpen && (
                  <div style={{ display: 'flex', gap: 8, marginTop: 12, paddingTop: 12, borderTop: '1px solid ' + T.line }}>
                    <button
                      ...
                      style={{ flex: 1, height: 38, borderRadius: 10, background: T.blueSoft, color: T.blue, fontSize: 13.5, fontWeight: 700 }}
                    >
                      수정
                    </button>
                    <button
                      onClick={() => { setDeleteTarget(g.id); setActiveMenu(null); }}
                      style={{ flex: 1, height: 38, borderRadius: 10, background: T.dangerSoft, color: T.danger, fontSize: 13.5, fontWeight: 700 }}
                    >
                      삭제
                    </button>
                  </div>
          ...
          <button onClick={() => setAddOpen(true)} style={{ height: 52, borderRadius: 14, border: '1.5px dashed #C6D3E6', color: T.blue, fontSize: 14.5, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, background: '#fff' }}>
