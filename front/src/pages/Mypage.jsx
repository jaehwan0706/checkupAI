...
export default function Mypage({ onNav, onLogout, toast, consent }) {
  ...
  return (
    ...
    <BottomSheet open={fontSizeSheet} onClose={() => setFontSizeSheet(false)}>
      <h3 style={{ margin: '0 0 6px 2px', fontSize: 18, fontWeight: 800, color: T.ink }}>글자 크기</h3>
      <p style={{ margin: '0 0 18px 2px', fontSize: 13, color: T.inkSoft, lineHeight: 1.5 }}>앱 전체의 글자 크기를 변경해요</p>
      <div style={{ display: 'flex', gap: 8, marginBottom: 18 }}>
        {[
          ...
          >
            ...
            <span style={{ fontSize: 12, fontWeight: 700, color: fontSize === key ? 'rgba(255,255,255,0.85)' : T.inkSoft }}>{label}</span>
          </button>
        ))}
      </div>
    </BottomSheet>
