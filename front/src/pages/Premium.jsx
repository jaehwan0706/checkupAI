...
function PlanFeature({ children, light }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '3px 0' }}>
      ...
      <span style={{ fontSize: '0.8125rem', fontWeight: 600, color: light ? '#fff' : T.ink }}>{children}</span>
    </div>
  );
}
  ...
  return (
    <Modal open={open} onClose={onClose}>
      <div style={{ textAlign: 'center' }}>
        ...
        <h3 style={{ margin: 0, fontSize: '1.125rem', fontWeight: 800, color: T.ink }}>프리미엄 기능이에요</h3>
        <p style={{ margin: '10px 0 20px', fontSize: '0.8438rem', color: T.inkMid, lineHeight: 1.6 }}>이 기능은 월간 패스에서 이용할 수 있어요. 지금 시작하고 모든 기능을 누려보세요.</p>
      </div>
      <Button variant="gold" onClick={onUpgrade} icon="crown">플랜 보기</Button>
      <button onClick={onClose} style={{ width: '100%', height: 48, marginTop: 6, fontSize: '0.875rem', fontWeight: 700, color: T.inkSoft }}>닫기</button>
    </Modal>
  );
}
  ...
  return (
    <div data-screen-label="프리미엄" className="nd-no-scrollbar" style={{ flex: 1, overflow: 'auto', background: T.bg, display: 'flex', flexDirection: 'column' }}>
      <div style={{ padding: '52px 24px 0', textAlign: 'center' }}>
        ...
        <h1 style={{ margin: 0, fontSize: '1.375rem', fontWeight: 800, letterSpacing: '-0.02em', color: T.ink }}>내 건강, 제대로 알아봐요</h1>
        <p style={{ margin: '7px 0 0', fontSize: '0.8125rem', color: T.inkSoft }}>AI가 분석한 나만의 건강 리포트</p>
      </div>
      ...
      <div style={{ padding: '20px 20px 0', display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div style={{ background: '#fff', borderRadius: 18, border: '1px solid ' + T.line, padding: 16 }}>
          <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 12 }}>
            <span style={{ fontSize: '0.9375rem', fontWeight: 800, color: T.ink }}>건당 해석</span>
            <span style={{ fontSize: '0.8438rem', fontWeight: 700, color: T.inkMid }}><b style={{ fontSize: '1.0625rem', color: T.ink }}>1,900원</b> / 1회</span>
          </div>
        ...
        <div style={{ position: 'relative', borderRadius: 18, padding: 18, background: 'linear-gradient(160deg,#1E4D8C 0%,#163A6B 100%)', boxShadow: '0 16px 36px rgba(30,77,140,0.34)', border: '1.5px solid #F0B445' }}>
          <div style={{ position: 'absolute', top: -11, left: '50%', transform: 'translateX(-50%)', background: 'linear-gradient(180deg,#F0B445,#E0982A)', color: '#3A2A06', fontSize: '0.6875rem', fontWeight: 800, padding: '4px 14px', borderRadius: 999, boxShadow: '0 6px 14px rgba(224,152,42,0.4)', whiteSpace: 'nowrap' }}>👑 가장 인기</div>
          <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginTop: 4 }}>
            <span style={{ fontSize: '1rem', fontWeight: 800, color: '#fff' }}>월간 패스</span>
            <span style={{ fontSize: '0.8438rem', fontWeight: 700, color: 'rgba(255,255,255,0.8)' }}><b style={{ fontSize: '1.1875rem', color: '#fff' }}>9,900원</b> / 월</span>
          </div>
          ...
          <div style={{ textAlign: 'center', marginBottom: 8, fontSize: '0.75rem', fontWeight: 600, color: 'rgba(255,255,255,0.85)' }}>이번 달 2,847명이 선택했어요 ⭐ 4.9</div>
          {isPremium
            ? <button disabled style={{ width: '100%', height: 50, borderRadius: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontSize: '0.9688rem', fontWeight: 800, background: 'linear-gradient(180deg,#F0B445,#E0982A)', color: '#3A2A06', boxShadow: '0 8px 18px rgba(224,152,42,0.36)', opacity: 0.85 }}>
      ...
      <div style={{ padding: '16px 24px 24px', textAlign: 'center', marginTop: 'auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 14, marginBottom: 12 }}>
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: '0.7188rem', color: T.inkSoft, fontWeight: 600 }}>
        ...
        {onNav && (
          <button onClick={() => { onClose(); onNav('premiumReport'); }} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: '0.8125rem', fontWeight: 700, color: T.blue, padding: '6px 0' }}>
            <Icon name="spark" size={14} color={T.blue} stroke={2.3} />
            샘플 AI 리포트 미리보기
            <Icon name="chevR" size={14} color={T.blue} />
          </button>
        )}
        <button onClick={onClose} disabled={paying !== null} style={{ fontSize: '0.8438rem', fontWeight: 700, color: T.inkSoft, padding: 8, opacity: paying ? 0.5 : 1 }}>나중에 하기</button>
      </div>
    </div>
  );
}
